...
const processFile = (filePath) => {
  ...
  const hasRequestOrResponse =
    updatedContent.includes("<Request>") ||
    updatedContent.includes("<Response>");
  const hasH2OrH3 = /^(##|###)\s/m.test(updatedContent);

  // Append the footer only if conditions are met
  if (hasRequestOrResponse && !hasH2OrH3) {
